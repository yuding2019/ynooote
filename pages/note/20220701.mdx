...
`message`允许传入字符串或者一个方法。
1. 如果是字符串，拦截提示就是浏览器原生的`window.confirm(message)`
2. 如果回调返回字符串就和传入`message`一样
...
和上述数组push拦截类似，在调用`history.push(url)`后，会执行类似的判断逻辑
...
`window.history.go()`调用后会触发一个`popstate`的事件，其中事件参数会携带激活url的相关信息。

利用这个事件可以实现`go(n)`的拦截，主要原理就是在跳转后依据`popstate`事件中相关数据计算出跳转的url，然后会经过和上面一样的判断过程。
